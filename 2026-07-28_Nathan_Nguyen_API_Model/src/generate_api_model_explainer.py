...
FOCUSED_TEMPLATE_PATH = (
    ...
    / "docs"
    / "assets"
    / "puerto-rico-api-model-focused.template.html"
)
FOCUSED_OUTPUT_PATH = PROJECT_ROOT / "docs" / "puerto-rico-api-model-focused.html"
...
def build_explainer(output_path: Path = OUTPUT_PATH) -> Path:
    ...
    if INFORMATION_AUDIT_PATH.exists():
        information_audit = json.loads(
            INFORMATION_AUDIT_PATH.read_text(encoding="utf-8")
        )
    else:
        information_audit = {
            "status": "not_run",
            "information_available_validation": metadata.get(
    ...
    replacements = {
        ...
        "__INFORMATION_AVAILABLE_AUDIT__": _safe_script_json(information_audit),
    }
    ...
    if (
        "__MODEL_" in page
        or "__VALIDATION_" in page
        or "__LIVE_" in page
        or "__INFORMATION_" in page
    ):
...
def build_focused_explainer(
    output_path: Path = FOCUSED_OUTPUT_PATH,
) -> Path:
    """Build the shorter explainer dedicated to the operational API model."""

    metadata = json.loads(CHAMPION_PATH.read_text(encoding="utf-8"))
    validation = pd.read_csv(VALIDATION_PATH)
    predictions = pd.read_csv(PREDICTIONS_PATH)
    information_audit = (
        json.loads(INFORMATION_AUDIT_PATH.read_text(encoding="utf-8"))
        if INFORMATION_AUDIT_PATH.exists()
        else {"information_available_validation": {"status": "not_run"}}
    )
    replacements = {
        "__MODEL_METADATA__": _safe_script_json(metadata),
        "__VALIDATION_ROWS__": _safe_script_json(_validation_records(validation)),
    ...
    page = FOCUSED_TEMPLATE_PATH.read_text(encoding="utf-8")
    for token, value in replacements.items():
        if token not in page:
            raise ValueError(f"Focused template token is missing: {token}")
        page = page.replace(token, value)
    if any(token in page for token in replacements):
        raise ValueError("One or more focused-explainer tokens were not replaced")
    output_path.write_text(page, encoding="utf-8")
    return output_path
