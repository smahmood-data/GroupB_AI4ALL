...
``audit``
    Re-run validation with archived weather forecasts that existed before each
    held-out week, without promoting or replacing the live champion.
``import-peru``
...
DEFAULT_INFORMATION_AUDIT = (
    DEFAULT_REPORTS / "pr_information_available_validation_latest.json"
)
DEFAULT_CACHE = PROJECT_ROOT / ".cache" / "near_realtime"
DEFAULT_VINTAGES = PROJECT_ROOT / "data" / "operational" / "vintages"
...
def ingest_sources(config: dict[str, Any]) -> dict[str, Any]:
    ...
    case_vintage_id, case_vintage_path, case_vintage_created = save_case_vintage(
        snapshot,
        DEFAULT_VINTAGES / "cases",
    )
    ...
    status: dict[str, Any] = {
        ...
        "puerto_rico": {
            ...
            "case_vintage_path": str(case_vintage_path.relative_to(PROJECT_ROOT)),
            "case_vintage_created": case_vintage_created,
        },
    }
    try:
...
def predict_and_monitor(
    ...
    new_rows = score_champion(
        ...
        as_of=as_of,
        refresh=refresh,
        input_vintage_dir=DEFAULT_VINTAGES / "model_inputs",
        case_vintage_id=case_vintage_id,
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "command",
        choices=["ingest", "weekly", "monthly", "monitor", "audit", "import-peru"],
    )
...
def main() -> None:
    ...
    if args.command in {"ingest", "weekly", "monthly", "audit"}:
    ...
    if args.command == "audit":
        ...
        _, metadata = train_candidate(stable, config, DEFAULT_CACHE, args.refresh)
        report = {
            "evaluated_at_utc": datetime.now(timezone.utc).isoformat(),
            ...
            "headline_validation_basis": metadata["validation_disclosure"][
                "headline_basis"
            ],
            "information_available_validation": metadata[
                "information_available_validation"
            ],
        }
        _write_json(DEFAULT_INFORMATION_AUDIT, report)
    ...
    if args.command in {"weekly", "monthly"}:
        predictions, metrics = predict_and_monitor(
            config,
            as_of,
            args.refresh,
            case_vintage_id=status["puerto_rico"].get("case_vintage_id"),
        )
        display = predictions[
            [
                "week_start_date",
