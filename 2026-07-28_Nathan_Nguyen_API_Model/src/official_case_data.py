...
def save_case_vintage(
    snapshot: PuertoRicoSnapshot,
    vintage_dir: Path,
) -> tuple[str, Path, bool]:
    """Persist the exact normalized case table available at one API issue time.

    The canonical ``pr_weekly.csv`` intentionally changes when the Department
    of Health revises older counts.  That behavior is correct for training, but
    it cannot answer the retrospective question "what did the model know on
    this date?"  This immutable vintage store can.

    The content identifier excludes ``retrieved_at_utc`` so polling the same
    official file twice does not create duplicate snapshots.  If the provider
    changes a file in place, the normalized weekly content changes and receives
    a new identifier even when the upstream file id is unchanged.
    """

    frame = snapshot.weekly_cases.reindex(columns=CASE_TABLE_COLUMNS).copy()
    frame["week_start_date"] = pd.to_datetime(frame["week_start_date"]).dt.date.astype(str)
    hash_columns = [column for column in CASE_TABLE_COLUMNS if column != "retrieved_at_utc"]
    canonical = frame[hash_columns].to_csv(index=False, lineterminator="\n")
    identity = {
        "source_file_id": snapshot.source_file_id,
        "publication_date": snapshot.publication_date,
        "weekly_content": canonical,
    }
    vintage_id = "pr-case-" + hashlib.sha256(
        json.dumps(identity, sort_keys=True).encode("utf-8")
    ).hexdigest()[:16]
    publication_day = pd.Timestamp(snapshot.publication_date).date().isoformat()
    path = vintage_dir / f"{publication_day}_{vintage_id}.csv"
    if path.exists():
        return vintage_id, path, False

    vintage_dir.mkdir(parents=True, exist_ok=True)
    frame.to_csv(path, index=False, lineterminator="\n")
    return vintage_id, path, True
