...
class NearRealtimeOutbreakTests(unittest.TestCase):
    ...
    def test_validation_overrides_drop_uncovered_weeks_and_replace_features(self) -> None:
        validation = pd.DataFrame(
            {
                "week_start_date": pd.to_datetime(["2024-03-11", "2024-03-18"]),
                "feature": [1.0, 2.0],
                "outbreak_label": [0, 1],
            }
        )
        overrides = pd.DataFrame(
            {
                "week_start_date": pd.to_datetime(["2024-03-18"]),
                "feature": [20.0],
            }
        )

        result = apply_validation_feature_overrides(
            validation, overrides, ["feature"]
        )

        self.assertEqual(len(result), 1)
        self.assertEqual(result.iloc[0]["week_start_date"], pd.Timestamp("2024-03-18"))
        self.assertEqual(result.iloc[0]["feature"], 20.0)
