...
from near_realtime_outbreak_detection import (
    ...
    apply_validation_feature_overrides,
...
def save_model_input_vintage(
    feature_row: pd.Series,
    feature_columns: list[str],
    metadata: dict[str, Any],
    vintage_dir: Path,
) -> tuple[str, Path]:
    """Save the exact processed inputs used for one issued prediction.

    Persisting model-ready features is more reproducible than relying on a
    mutable API or a transient cache.  The content hash omits the retrieval
    timestamp, so an identical same-day rerun reuses the same immutable file.
    Signed API URLs and other transient credentials are never included.
    """

    values = {
        column: _json_safe_scalar(feature_row.get(column))
    ...
    vintage_id = "pr-input-" + hashlib.sha256(
    ...
    path = vintage_dir / f"{vintage_id}.json"
    if not path.exists():
        vintage_dir.mkdir(parents=True, exist_ok=True)
        _write_json(path, payload)
    return vintage_id, path
...
def parse_issue_time_hourly_weather(
    payload: dict[str, Any],
    target_week_start: pd.Timestamp,
    source: str,
) -> pd.DataFrame:
    """Aggregate one archived forecast run into seven local daily rows.

    Single-run daily output requires the model run to begin at midnight in the
    requested local timezone, which is not a valid ECMWF cycle for Puerto Rico.
    Hourly output avoids that ambiguity and lets us aggregate the exact UTC run
    into local Monday-through-Sunday days ourselves.
    """

    hourly = payload.get("hourly")
    if not isinstance(hourly, dict) or "time" not in hourly:
        raise ValueError("Archived forecast response is missing hourly data")
    missing = [name for name in ISSUE_TIME_HOURLY_COLUMNS if name not in hourly]
    if missing:
        raise ValueError(f"Archived forecast response is missing: {missing}")
    frame = pd.DataFrame({"time": pd.to_datetime(hourly["time"], errors="raise")})
    for column in ISSUE_TIME_HOURLY_COLUMNS:
        if len(hourly[column]) != len(frame):
            raise ValueError(f"Archived forecast has inconsistent {column} length")
        frame[column] = pd.to_numeric(hourly[column], errors="coerce")
    start = pd.Timestamp(target_week_start).normalize()
    end = start + pd.Timedelta(days=6)
    frame["date"] = frame["time"].dt.normalize()
    frame = frame[(frame["date"] >= start) & (frame["date"] <= end)].copy()
    if frame["date"].nunique() != 7:
        raise ValueError("Archived forecast does not cover all seven target-week days")
    complete_hours = frame.groupby("date")[ISSUE_TIME_HOURLY_COLUMNS].count()
    if (complete_hours < 24).any().any():
        raise ValueError("Archived forecast has incomplete hourly target-week inputs")
        ...
        records.append(
            {
                ...
                "temperature_2m_max": float(group["temperature_2m"].max()),
                "temperature_2m_min": float(group["temperature_2m"].min()),
                "precipitation_sum": float(group["precipitation"].sum()),
                "rain_sum": float(group["rain"].sum()),
                # Match Open-Meteo's daily precipitation-hours convention by
                # counting hours with at least 0.1 mm of precipitation.
                "precipitation_hours": float((group["precipitation"] >= 0.1).sum()),
                "relative_humidity_2m_mean": float(
                    group["relative_humidity_2m"].mean()
                ),
                "dew_point_2m_mean": float(group["dew_point_2m"].mean()),
                "soil_moisture_0_to_7cm_mean": float(
                    group["soil_moisture_0_to_7cm"].mean()
                ),
                "et0_fao_evapotranspiration": float(
                    group["et0_fao_evapotranspiration"].sum()
                ),
...
def fetch_island_issue_time_weather(
    target_starts: list[pd.Timestamp],
    config: dict[str, Any],
    cache_dir: Path,
    refresh: bool,
) -> tuple[pd.DataFrame, str]:
    """Fetch archived forecasts that were issued before each validation week."""

    policy = config.get("information_available_validation", {})
    if not policy.get("enabled", False):
        return pd.DataFrame(), "disabled"
    available_from = pd.Timestamp(policy["archived_weather_available_from"])
    targets = sorted(
        pd.Timestamp(value).normalize()
        for value in target_starts
        if pd.Timestamp(value).normalize() >= available_from
    )
    settings = config["puerto_rico"]
    states: set[str] = set()
    weekly_parts: list[pd.DataFrame] = []
    issue_days = int(policy.get("issue_days_before_target_week", 1))
    run_hour = int(policy.get("issue_run_hour_utc", 0))
    forecast_hours = int(policy.get("forecast_hours", 240))
    model = str(policy.get("archived_weather_model", "ecmwf_ifs"))
    points = [
        _point_config(raw_point, settings["timezone"])
        for raw_point in settings["weather_points"]
    ]

    def fetch_target(target: pd.Timestamp) -> tuple[pd.DataFrame | None, str]:
        """Fetch and aggregate one target week across all configured points."""

        run_day = target - pd.Timedelta(days=issue_days)
        run = f"{run_day.date().isoformat()}T{run_hour:02d}:00"
        # Open-Meteo accepts comma-separated coordinates and returns one
        # payload per location.  Fetching the six island points together cuts
        # a clean historical audit from six requests per week to one without
        # changing any spatial aggregation or completeness checks.
        parameters = {
            "latitude": ",".join(str(point.latitude) for point in points),
            "longitude": ",".join(str(point.longitude) for point in points),
            "run": run,
            "forecast_hours": forecast_hours,
            "hourly": ",".join(ISSUE_TIME_HOURLY_COLUMNS),
        ...
        try:
            response = fetch_json_with_cache(
                ...
                parameters,
                cache_dir,
                cache_prefix=f"pr_island_issued_{target.date().isoformat()}",
                max_cache_age_hours=None,
                refresh=refresh,
            )
        except RuntimeError as exc:
            # The single-runs archive has occasional missing early runs. A 400
            # means that exact requested run is unavailable. Exclude the week
            # from the audit; never replace it with finalized historical data.
            if "HTTP Error 400" in str(exc):
                return None, "unavailable_archived_run"
            raise RuntimeError(
                f"Unable to fetch archived weather for {target.date()}: {exc}"
            ) from exc
        payloads = response.payload
        if not isinstance(payloads, list) or len(payloads) != len(points):
            raise ValueError(
                "Archived multi-location forecast did not return one payload "
                "for every configured Puerto Rico weather point"
            )
        point_frames: list[pd.DataFrame] = []
        for point, payload in zip(points, payloads, strict=True):
            try:
                daily = parse_issue_time_hourly_weather(
                    payload,
                    target,
                    source=f"open_meteo_single_run:{model}:{run}:{point.code}",
                )
            except ValueError as exc:
                # Some archived cycles exist but contain a partial horizon for
                # one location. The island week is unusable unless every point
                # has every target-hour value, so exclude the complete week.
                if (
                    "does not cover all seven" in str(exc)
                    or "incomplete hourly" in str(exc)
                ):
                    return None, "incomplete_archived_run"
                raise
            daily["weather_point"] = point.code
            point_frames.append(daily)
        island_daily = spatially_aggregate_daily_weather(
            point_frames, len(settings["weather_points"])
        )
        weekly = aggregate_daily_to_weeks(island_daily, [target])
        weekly["forecast_issue_run_utc"] = run
        return weekly, response.cache_state

    if not targets:
        return pd.DataFrame(), "no_covered_weeks"

    # Four simultaneous weeks keep the audit practical while staying modest
    # enough for the public endpoint. Each response already bundles all six
    # locations, and immutable cache files make interrupted runs resumable.
    worker_count = min(4, len(targets))
    with ThreadPoolExecutor(max_workers=worker_count) as executor:
        for weekly, state in executor.map(fetch_target, targets):
            if weekly is not None:
                weekly_parts.append(weekly)
            states.add(state)
    if not weekly_parts:
        return pd.DataFrame(), "+".join(sorted(states)) or "no_covered_weeks"
    return pd.concat(weekly_parts, ignore_index=True), "+".join(sorted(states))


def build_issue_time_weather_feature_overrides(
    observed_weekly: pd.DataFrame,
    issued_weekly: pd.DataFrame,
) -> pd.DataFrame:
    """Recompute each held-out row using forecast weather for its target week.

    Weather from weeks before the target was already observable at issue time,
    so it remains historical.  Only the target week's raw weather is replaced;
    lag-zero and rolling features are then rebuilt from that mixed, realistic
    information set.
    """

    if issued_weekly.empty:
        return pd.DataFrame()
    base = observed_weekly.sort_values("week_start_date").copy()
    base["week_start_date"] = pd.to_datetime(base["week_start_date"]).dt.normalize()
    raw_columns = MODEL_WEATHER_COLUMNS
    feature_columns = sorted(
        set(weather_feature_columns() + case_forecast_weather_feature_columns())
    )
    rows: list[pd.Series] = []
    for _, issued in issued_weekly.sort_values("week_start_date").iterrows():
        target = pd.Timestamp(issued["week_start_date"]).normalize()
        changed = base.copy()
        mask = changed["week_start_date"] == target
        if mask.sum() != 1:
            continue
        for column in raw_columns:
            changed.loc[mask, column] = issued[column]
        featured = add_weather_features(changed)
        selected = featured[featured["week_start_date"] == target]
        if len(selected) != 1:
            continue
        result = selected.iloc[0][["week_start_date"] + feature_columns].copy()
        result["forecast_issue_run_utc"] = issued["forecast_issue_run_utc"]
        rows.append(result)
    if not rows:
        return pd.DataFrame(columns=["week_start_date"] + feature_columns)
...
def _calibrate_residual_scales(
    ...
    validation_feature_overrides: pd.DataFrame | None = None,
) -> dict[float, float]:
    ...
    calibration = apply_validation_feature_overrides(
        calibration,
        validation_feature_overrides,
        feature_columns,
    )
...
def fit_time_aware_case_forecaster(
    ...
    for validation_year in validation_years:
        ...
        validation = apply_validation_feature_overrides(
            validation,
            validation_feature_overrides,
            feature_columns,
        )
    ...
    metrics["validation_input_basis"] = (
        "archived_issue_time_weather"
        if validation_feature_overrides is not None
        else "finalized_historical_weather_proxy"
    )
...
def train_candidate(
    ...
    # The champion remains calibrated on the established retrospective folds
    # until enough prospective predictions mature.  In parallel, this audit
    # substitutes weather from archived model runs that existed before each
    # held-out target week.  Keeping it separate prevents a different archived
    # provider from being mislabeled as the live production score.
    information_available_audit: dict[str, Any] = {
        "status": "disabled",
        "weather_basis": "not_requested",
        "case_availability_basis": {
            "weather_only": "no_case_input",
            "weather_plus_delayed_cases": "simulated_fixed_report_delay",
            "weather_plus_recent_cases": "conditional_exact_history_not_vintage_verified",
        },
    }
    policy = config.get("information_available_validation", {})
    if policy.get("enabled", False):
        issued_weekly, issued_cache_state = fetch_island_issue_time_weather(
            labeled["week_start_date"].tolist(),
            config,
            cache_dir,
            refresh,
        )
        issue_time_overrides = build_issue_time_weather_feature_overrides(
            weekly_weather,
            issued_weekly,
        )
        if issue_time_overrides.empty:
            information_available_audit = {
                **information_available_audit,
                "status": "no_archived_weeks",
                "weather_basis": "archived_ecmwf_single_runs",
                "weather_cache_state": issued_cache_state,
            }
        else:
            audit_weather_detector = fit_time_aware_detector(
                classifier_training,
                weather_columns,
                target_recall=float(config["target_recall"]),
                validation_feature_overrides=issue_time_overrides,
            )
            audit_case_detector = fit_time_aware_detector(
                case_training,
                weather_columns + EXACT_HEALTH_FEATURE_COLUMNS,
                target_recall=float(config["target_recall"]),
                validation_feature_overrides=issue_time_overrides,
            )
            audit_delayed_detector = fit_time_aware_detector(
                delayed_classifier_training,
                weather_columns + DELAYED_HEALTH_FEATURE_COLUMNS,
                target_recall=float(config["target_recall"]),
                validation_filter=("report_age_weeks", float(validation_delay)),
            ...
            audit_weather_forecaster = fit_time_aware_case_forecaster(
                regression_training,
                case_forecast_weather_columns,
                variant="weather_only",
                validation_feature_overrides=issue_time_overrides,
            )
            audit_recent_forecaster = fit_time_aware_case_forecaster(
                regression_training.dropna(subset=EXACT_HEALTH_FEATURE_COLUMNS),
                case_forecast_weather_columns + EXACT_HEALTH_FEATURE_COLUMNS,
                variant="weather_plus_recent_cases",
                baseline_column="cases_lag_1",
            ...
            audit_delayed_forecaster = fit_time_aware_case_forecaster(
                delayed_regression_training.dropna(
                    subset=DELAYED_HEALTH_FEATURE_COLUMNS
                ),
                case_forecast_weather_columns + DELAYED_HEALTH_FEATURE_COLUMNS,
                variant="weather_plus_delayed_cases",
                baseline_column="delayed_cases_latest",
            ...
            information_available_audit = {
                **information_available_audit,
                "status": "complete",
                "weather_basis": "archived_ecmwf_single_runs_issued_before_target_week",
                "weather_cache_state": issued_cache_state,
                "issue_days_before_target_week": int(
                    policy.get("issue_days_before_target_week", 1)
                ),
                "issue_run_hour_utc": int(policy.get("issue_run_hour_utc", 0)),
                "archived_weather_model": str(
                    policy.get("archived_weather_model", "ecmwf_ifs")
                ),
                "eligible_archived_weeks": int(
                    (
                        labeled["week_start_date"]
                        >= pd.Timestamp(policy["archived_weather_available_from"])
                    ).sum()
                ),
                "covered_weeks": int(len(issue_time_overrides)),
                "uncovered_archived_weeks": int(
                    (
                        labeled["week_start_date"]
                        >= pd.Timestamp(policy["archived_weather_available_from"])
                    ).sum()
                    - len(issue_time_overrides)
                ),
                "comparison_note": (
                    "Archived metrics use only fully covered weeks; headline proxy "
                    "metrics use the full historical validation sample, so differences "
                    "are not a paired estimate of weather-provider impact"
                ),
                "coverage_start": pd.Timestamp(
                    issue_time_overrides["week_start_date"].min()
                ).date().isoformat(),
                "coverage_end": pd.Timestamp(
                    issue_time_overrides["week_start_date"].max()
                ).date().isoformat(),
                "classifier_metrics": {
                    "weather_only": audit_weather_detector.validation_metrics,
                    "weather_plus_delayed_cases": (
                        audit_delayed_detector.validation_metrics
                    ),
                    "weather_plus_recent_cases": audit_case_detector.validation_metrics,
                },
                "case_forecast_metrics": {
                    "weather_only": audit_weather_forecaster.validation_metrics,
                    "weather_plus_delayed_cases": (
                        audit_delayed_forecaster.validation_metrics
                    ),
                    "weather_plus_recent_cases": (
                        audit_recent_forecaster.validation_metrics
                    ),
                },
            }
    ...
    metadata = {
        ...
        "validation_disclosure": {
            "headline_basis": "finalized_historical_weather_proxy",
            "historical_case_vintages_available": False,
            "prospective_basis": "persisted_issue_time_inputs_after_this_version",
        },
        "information_available_validation": information_available_audit,
    }
...
def score_champion(
    ...
    refresh: bool = False,
    input_vintage_dir: Path | None = None,
    case_vintage_id: str | None = None,
) -> pd.DataFrame:
    """Generate current/next-week alerts and preserve their exact inputs."""
    ...
    latest_case_row = all_cases.loc[
        pd.to_datetime(all_cases["week_start_date"]) == latest_case
    ].iloc[-1]
    source_file_id = str(latest_case_row["source_file_id"])
    ...
    for lead, target in enumerate(target_starts):
        ...
        input_vintage_id: str | None = None
        if input_vintage_dir is not None:
            input_feature_columns = sorted(
                set(detector.feature_columns + case_forecaster.feature_columns)
            )
            input_metadata = {
                ...
                "case_vintage_id": case_vintage_id,
                "case_source_file_id": source_file_id,
                "case_source_publication_date": source_publication,
            ...
            input_vintage_id, _ = save_model_input_vintage(
                row,
        ...
        rows.append(
            {
                ...
                "forecast_input_days": int(row["forecast_input_days"]),
                "weather_sources": row["weather_sources"],
                "input_vintage_id": input_vintage_id,
                "input_vintage_basis": (
                    "prospective_persisted_issue_time_inputs"
                    if input_vintage_id is not None
                    else "not_persisted"
                ),
                "actual_cases": np.nan,
...
def _prospective_prediction_metrics(
    evaluated: pd.DataFrame,
    evaluated_at: str,
) -> dict[str, Any]:
    """Score only forecasts whose complete issue-time inputs were preserved."""

    if evaluated.empty:
        return {
            "evaluation_basis": "prospective_persisted_issue_time_inputs",
            "evaluated_rows": 0,
            "message": (
                "No persisted-input predictions have finalized outcomes yet; "
                "prospective validation starts with this pipeline version"
            ),
        }
    ...
    negatives = result["true_negatives"] + result["false_positives"]
    result["specificity"] = (
        float(result["true_negatives"] / negatives) if negatives else None
    )
    case_rows = evaluated[evaluated["predicted_cases_p50"].notna()].copy()
    if len(case_rows):
        actual = case_rows["actual_cases"].astype(float)
        p50 = case_rows["predicted_cases_p50"].astype(float)
        p80 = case_rows["predicted_cases_p80"].astype(float)
        p90 = case_rows["predicted_cases_p90"].astype(float)
        outbreak = case_rows["actual_outbreak"].astype(int) == 1
        result["case_forecast"] = {
            "evaluated_rows": int(len(case_rows)),
            "mae": float(mean_absolute_error(actual, p50)),
            "median_absolute_error": float(median_absolute_error(actual, p50)),
            "normal_week_mae": (
                float(mean_absolute_error(actual[~outbreak], p50[~outbreak]))
                if (~outbreak).any()
                else None
            ),
            "outbreak_week_mae": (
...
def reconcile_predictions(
    ...
    if not prediction_path.exists():
        metrics = {
            "evaluated_rows": 0,
            "message": "No predictions exist yet",
            "prospective_information_available": _prospective_prediction_metrics(
                pd.DataFrame(), datetime.now(timezone.utc).isoformat()
            ),
        }
    ...
    metrics["evaluation_basis"] = (
        "all_versioned_predictions; use prospective_information_available "
        "for the leakage-resistant deployment score"
    )
    prospective = evaluated[
        evaluated["input_vintage_basis"]
        == "prospective_persisted_issue_time_inputs"
    ].copy()
    metrics["prospective_information_available"] = _prospective_prediction_metrics(
        prospective, evaluated_at
    )
    _write_json(metrics_path, metrics)
    return metrics
