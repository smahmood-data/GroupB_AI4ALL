...
from puerto_rico_operational import (  # noqa: E402
    ...
    build_issue_time_weather_feature_overrides,
    evaluate_promotion,
    finalized_cases,
    live_delayed_health_features,
    parse_issue_time_hourly_weather,
    reconcile_predictions,
    save_model_input_vintage,
...
class OfficialCaseDataTests(unittest.TestCase):
    ...
    def test_case_vintage_is_immutable_and_deduplicates_identical_api_files(self) -> None:
        dates = pd.date_range("2026-01-05", periods=7, freq="D")
        records = [
            {
                "diagnosticDate": day.date().isoformat(),
                "totalCasesPcrCount": 1,
                "totalCasesIgMCount": 1,
                "totalHospitalizedCount": 0,
            }
            for day in dates
        ]
        weekly = normalize_puerto_rico_daily_cases(
        ...
        with tempfile.TemporaryDirectory() as directory:
            first_id, first_path, first_created = save_case_vintage(
                snapshot, Path(directory)
            )
            second_id, second_path, second_created = save_case_vintage(
                snapshot, Path(directory)
            )
...
class PuertoRicoOperationalTests(unittest.TestCase):
    ...
    def test_archived_hourly_forecast_is_aggregated_to_target_week_days(self) -> None:
        times = pd.date_range("2024-07-01", periods=7 * 24, freq="h")
        payload = {
            "hourly": {
                "time": times.strftime("%Y-%m-%dT%H:%M").tolist(),
                "temperature_2m": [24.0] * len(times),
                "precipitation": [0.2] * len(times),
                "rain": [0.2] * len(times),
                "relative_humidity_2m": [80.0] * len(times),
                "dew_point_2m": [20.0] * len(times),
                "soil_moisture_0_to_7cm": [0.3] * len(times),
                "et0_fao_evapotranspiration": [0.1] * len(times),
            }
        }

        daily = parse_issue_time_hourly_weather(
            payload, pd.Timestamp("2024-07-01"), "archived-test"
        )

        self.assertEqual(len(daily), 7)
        self.assertAlmostEqual(daily.loc[0, "precipitation_sum"], 4.8)
        self.assertEqual(daily.loc[0, "precipitation_hours"], 24.0)
        self.assertEqual(daily.loc[0, "weather_source"], "archived-test")

    def test_issue_time_override_replaces_target_weather_but_keeps_past_lags(self) -> None:
        starts = pd.date_range("2024-04-01", periods=12, freq="7D")
        observed = pd.DataFrame({"week_start_date": starts})
        for index, column in enumerate(MODEL_WEATHER_COLUMNS):
            observed[column] = np.arange(12, dtype=float) + index
        target = starts[-1]
        issued = observed[observed["week_start_date"] == target].copy()
        issued["temperature_2m_mean"] = 100.0
        issued["forecast_issue_run_utc"] = "2024-06-09T00:00"

        overrides = build_issue_time_weather_feature_overrides(observed, issued)

        self.assertEqual(len(overrides), 1)
        self.assertEqual(overrides.loc[0, "temperature_2m_mean_lag_0"], 100.0)
        self.assertEqual(
            overrides.loc[0, "temperature_2m_mean_lag_2"],
            observed.loc[9, "temperature_2m_mean"],
        )

    def test_model_input_vintage_is_content_addressed(self) -> None:
        row = pd.Series({"feature_a": 1.5, "feature_b": 2.0})
        metadata = {
            "generated_at_utc": "2026-07-21T12:00:00Z",
            "as_of_date": "2026-07-21",
            "week_start_date": "2026-07-20",
            "model_variant": "weather_plus_delayed_cases",
        }
        with tempfile.TemporaryDirectory() as directory:
            first_id, first_path = save_model_input_vintage(
                row, ["feature_a", "feature_b"], metadata, Path(directory)
            )
            second_id, second_path = save_model_input_vintage(
                row, ["feature_b", "feature_a"], metadata, Path(directory)
            )
    ...
    def test_monitoring_discloses_empty_prospective_score_from_day_one(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            prospective = metrics["prospective_information_available"]
            self.assertEqual(prospective["evaluated_rows"], 0)
            self.assertEqual(
                prospective["evaluation_basis"],
