...
def apply_validation_feature_overrides(
    validation: pd.DataFrame,
    overrides: pd.DataFrame | None,
    feature_columns: list[str],
) -> pd.DataFrame:
    """Replace held-out features with values available at forecast issue time.

    The override table is intentionally keyed only by ``week_start_date``.  A
    delayed-case validation frame may contain several simulated report ages for
    one target week, so the merge is many-to-one.  Weeks without a complete
    archived input vintage are excluded instead of silently falling back to
    finalized target-week weather.
    """

    if overrides is None:
        return validation.copy()
    override_columns = [
        column for column in feature_columns if column in overrides.columns
    ]
    if not override_columns:
        raise ValueError("Validation overrides do not contain any model features")
    available = overrides[["week_start_date"] + override_columns].copy()
    if available.duplicated("week_start_date").any():
        raise ValueError("Validation overrides contain duplicate target weeks")
    renamed = {
        column: f"{column}__issue_time" for column in override_columns
    }
    merged = validation.merge(
        available.rename(columns=renamed),
        on="week_start_date",
        how="left",
        validate="many_to_one",
    )
    issued_columns = list(renamed.values())
    merged = merged.dropna(subset=issued_columns).copy()
    # Replace the feature block in one concatenation. Assigning more than 100
    # weather columns one by one fragments pandas' internal storage and makes
    # the repeated time folds substantially slower.
    issue_time_features = merged[issued_columns].rename(
        columns={issued: column for column, issued in renamed.items()}
    )
    retained = merged.drop(columns=override_columns + issued_columns)
    return pd.concat(
        [retained.reset_index(drop=True), issue_time_features.reset_index(drop=True)],
        axis=1,
    )
